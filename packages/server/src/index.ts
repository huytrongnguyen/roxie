...
export async function launchServer(config: AppConfig = {}) {
    ...
    routes.forEach((route: any) => {
      ...
      (<any>app)[httpMethod](path, async(req: express.Request, res: express.Response) => {
        try {
          ...
          Roxie.Logger.logError(err.message);
  ...
  app.listen(portNumber, (err: any) => {
    if (err) {
      Roxie.Logger.logError('Unable to start Express.');
    } else {
      Roxie.Logger.logInfo(`Content root path: ${rootPath}`);
      Roxie.Logger.logInfo(`Now listening on: http://localhost:${portNumber}`);
      Roxie.Logger.logInfo('Application started. Press Ctrl+C to shut down.');
